...
jQuery.fn.daterangepicker = function(settings){
	...
	var rpPresets = (function(){
		...
		//點擊今天
		ul.find('.ui-daterangepicker-今天').click(function(){			
			rp.find('.ui-state-active').removeClass('ui-state-active');
			jQuery(this).addClass('ui-state-active');
			clickActions(jQuery(this),rp, rpPickers, doneBtn);
			hideRP();			
	...
	function clickActions(el, rp, rpPickers, doneBtn){
		...
		if(el.is('.ui-daterangepicker-specificDate')){
			...
			rp.find('.range-end').restoreDateFromData().css('opacity',0).hide();
		...
		$('.range-start').datepicker('setDate', parsedDate);			
		if(input.val().split("-")[1]){
			var enddate = input.val().split("-")[1].replace(/\//g,'-').replace(/\ /,'');			
			parsedDate = $.datepicker.parseDate('yy-mm-dd', enddate);
			$('.range-end').datepicker('setDate', parsedDate);		
		}		
		return false;
	}	
